...
data=data.frame(Title=NA, Date=NA, Price=NA, URL=NA)
...
select$price=as.numeric(gsub('[^0-9]','',select$price))
select=select[select$price>10000&select$price<40000,]
dim(select)
...
reg=subset(i64, select=c(price,D.value))
model=lm(price ~ D.value, data = reg)
reg=subset(fortify(model),select=c(price,D.value,.fitted))
names(reg)[3]='FIT'
i64=  join(i64,reg);head(i64)

reg=subset(i128, select=c(price,D.value))
model=lm(price ~ D.value, data = reg)
reg=subset(fortify(model),select=c(price,D.value,.fitted))
names(reg)[3]='FIT'
i128=  join(i128,reg);head(i128)

reg=subset(i16, select=c(price,D.value))
model=lm(price ~ D.value, data = reg)
reg=subset(fortify(model),select=c(price,D.value,.fitted))
names(reg)[3]='FIT'
...
# 
# library(ggplot2)
# g=ggplot(data=select,aes(x=Date,y=price,group=1))+geom_point(colour='red',alpha=.8)+stat_smooth(method='glm')+geom_line()+
...
p16=subset(i16,select=c('D2','price','Title','URL'))
p64=subset(i64,select=c('D2','price','Title','URL'))
p128=subset(i128,select=c('D2','price','Title','URL'))
...
hm$tooltip(borderWidth=0, followPointer=TRUE, followTouchMove=TRUE, shared = FALSE,formatter = "#! function(){return this.point.z + '<br>'+ 'price:' + this.point.y ;} !#")
hm$plotOptions(
  scatter = list(
    cursor = "pointer", 
    point = list(
...
#hm$publish('iPhone6s_priceTrend', host ='rpubs', user='Poissonfish')
